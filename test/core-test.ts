...
describe("Core tests", () => {
    ...
    describe("#vote()", () => {
        ...
        it("An user should not vote on a non-existent ballot", async () => {
            ...
            const proof = await createSnknotProof(voterPublicKeys, ballotIndex, accounts[1].voter, vote)
        ...
        it("An user should not vote on ballot with a wrong smt root", async () => {
            ...
            const proof = await createSnknotProof(wrongVoterPublicKeys, ballotIndex, accounts[1].voter, vote)
        ...
        for (let i = 1; i < 5; i++) {
            it(`User ${i} should vote anonymously on ballot 0`, async () => {
                const ballotIndex = 0n
                const proof = await createSnknotProof(voterPublicKeys, ballotIndex, accounts[i].voter, vote)
        ...
        it("An user should not vote with a invalid proof", async () => {
            const ballotIndex = 0n
            const proof = await createSnknotProof(voterPublicKeys, ballotIndex, accounts[0].voter, vote)
